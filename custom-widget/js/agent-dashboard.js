...
class AgentDashboard {
    ...
    async loadCurrentSettings() {
        try {
            ...
            document.getElementById('current-provider').textContent = health.aiProvider?.provider || 'N/A';
            document.getElementById('current-model').textContent = health.aiProvider?.provider === 'openrouter' ? 'Gemini 2.5 Flash' : 'Flowise Model';
            document.getElementById('provider-status').textContent = health.aiProvider?.healthy ? '✅ Healthy' : '❌ Unhealthy';

            // Set radio button
            const currentProvider = health.aiProvider?.provider;
            if (currentProvider) {
                const radio = document.querySelector(`input[name="ai-provider"][value="${currentProvider}"]`);
                if (radio) radio.checked = true;
            }

            // Get current system prompt from backend (we'll need to add an endpoint for this)
    ...
    async loadSystemPrompt() {
        try {
            const response = await fetch(`${this.apiUrl}/api/config/system-prompt`);
            if (response.ok) {
            ...
            document.getElementById('system-prompt').value = 'Jūs esate naudingas klientų palaikymo asistentas Vilniaus miestui. Atsakykite lietuvių kalba ir būkite mandagūs bei informatyvūs.';
        }
    }
    ...
    async saveSettings() {
        try {
            ...
            this.showNotification('Klaida išsaugant nustatymus', 'error');
        }
    }
